...
def index():
    ...
    if request.method == "POST":
        ...
        # 1️⃣ Cari place_id dulu dengan Google Maps
        search = GoogleSearch({
            "engine": "google_maps",
            "q": product_name,
            "type": "search",
        ...
        if place_id:
            # 2️⃣ Ambil review dengan google_maps_reviews
            ...
            # 3️⃣ Analisis sentimen setiap review
            if reviews:
                for r in reviews:
                    text = r.get("snippet", "")
                    if text.strip():
